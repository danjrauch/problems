...
vector<point> sortPolygon(vector<point> poly, int type){
...
int main(){
  ...
  vector<point> P = inputPolygon(n);
  vector<point> Q = inputPolygon(n);
  vector<point> PQ;
  for(int i = 0; i<P.size()-1; ++i) PQ.push_back(P[i]);
  for(int i = 0; i<Q.size()-1; ++i) PQ.push_back(Q[i]);
  PQ.push_back(PQ[0]);
  P = sortPolygon(P, COUNTERCLOCKWISE);
  Q = sortPolygon(Q, COUNTERCLOCKWISE);
  printf("unsorted PQ\n");
  for(int i = 0; i<PQ.size(); ++i){
    printf("(%lf %lf)\n", PQ[i].x, PQ[i].y);
  }
  PQ = sortPolygon(PQ, COUNTERCLOCKWISE);
  printf("sorted PQ\n");
  for(int i = 0; i<PQ.size(); ++i){
    printf("(%lf %lf)\n", PQ[i].x, PQ[i].y);
  }
